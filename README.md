...
Builds **`Gudhi.xcframework`** — a macOS (arm64) static-library xcframework that
...
| repo | role |
|------|------|
| `cpp/gudhi-xcframework-builder` (this) | compiles the facade + GUDHI → `Gudhi.xcframework` |
...
- `Headers/GUDHI_PROVENANCE.txt` (shipped inside the xcframework) and
  `output/Gudhi.xcframework.provenance.txt` (tag, version, commit).
...
Output: `output/Gudhi.xcframework` (+ `.zip` and `.zip.sha256` for a future
...
4. **Assemble** `Headers/` (umbrella + per-module facade headers + `module.modulemap`
   + provenance).
5. **`xcodebuild -create-xcframework`** → `Gudhi.xcframework`.
6. **Zip + checksum**.
